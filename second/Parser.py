class Node:
    ...
    def __str__(self):
        return f"value: {self.value}, type:{type}"
        ...
        if self.lexemes[self.position].value == "for" and self.lexemes[self.position].type == "Keyword":
            ...
            if self.lexemes[self.position].type == "Identifier":
                ...
                if self.lexemes[self.position].value == "in" and self.lexemes[self.position].type == "Keyword":
                    ...
                    self.position += 1
                    if self.lexemes[self.position].value == "range" and self.lexemes[self.position].type == "Identifier":
                        children.append(Node("range", "Identifier"))
                        self.position += 1
                        if self.lexemes[self.position].value == "(" and self.lexemes[self.position].type == "LeftParenthesis":
                            children.append(Node("(", "LeftParenthesis"))
                            self.position += 1
                            if self.lexemes[self.position].type == "Identifier":
                                children.append(Node(self.lexemes[self.position].value, "Identifier"))
                                self.position += 1
                                if self.lexemes[self.position].value == ")" and self.lexemes[self.position].type == "RightParenthesis":
                                    children.append(Node(")", "RightParenthesis"))
                                    self.position += 1
                                    if self.lexemes[self.position].value == ":" and self.lexemes[self.position].type == "Colon":
